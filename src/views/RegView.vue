...
import { createUserFromAuthenticatedUser, type AuthenticatedUser, type User } from '@/models';
...
const authenticatedUser: AuthenticatedUser = JSON.parse(localStorage.getItem('user') || '{}');
...
function gotoGitlab() {
  window.open(
    import.meta.env.VITE_URL_REG_REDIRECT,
    "_self"
  );
}
    ...
    <n-card class="card container">
      ...
      <div class="content">
        ...
        <div v-if="currentStep === 2" class="content_container">
          ...
          <n-form label-placement="left" :model="user" :rules="formValidationRules" ref="formRef">
            <n-form-item :label="$t('regview.label.name')" path="name" label-placement="left">
              <n-input disabled :value="user.name" />
            </n-form-item>
            <n-form-item :label="$t('regview.label.email')" path="email" label-placement="left">
              <n-input disabled :value="user.email" />
            </n-form-item>
            <n-form-item :label="$t('regview.label.stuid')" path="thuid" label-placement="left">
              <n-input disabled :value="user.thuid" />
            </n-form-item>
            <n-form-item :label="$t('regview.label.phone')" path="phone" :required="true">
              <n-input v-model:value="user.phone" :allow-input="(input) => {
                return /^[0-9]*$/.test(input);
              }" />
            </n-form-item>
            <n-form-item :label="$t('regview.label.department')" path="department" :required="true">
              <n-input v-model:value="user.department" />
            </n-form-item>
            <n-form-item :label="$t('regview.label.qq')" path="qq" :required="true">
              <n-input v-model:value="user.qq" :allow-input="(input) => {
                return /^[0-9]*$/.test(input);
              }" />
            </n-form-item>
            <n-form-item>
              <n-button :loading="registering" :disabled="registering" size="large" type="primary" @click="submitForm"
                circle class="submit_btn">
                {{ $t('regview.submit') }}
              </n-button>
            </n-form-item>
          </n-form>
